...
if(True):
	...
	tk.Label(barraTitulo, text="ORDEN DE CARGA", font=("Helvetica Neue",12,"bold"), anchor="n", backgroun="#242b33", foreground = "#ffffff").pack()
...
if(True):
	...
	if(True):
		...
		tabla = ttk.Treeview(lbl_tabla, columns=["CORRAL", "CLIENTE", "CATEGORIA", "PINTURA", "CANTIDAD", "ORDEN"], selectmode=tk.BROWSE, show='headings') 
		...
		tabla.heading("ORDEN", text="ORDEN", command=lambda: treeview_sort_column(tabla, "ORDEN", False))
	...
	if(True):
		...
		tablaGastos = ttk.Treeview(lbl_tablaGastos, columns=["ORDEN", "NUMERO", "DESCRIPCION"], selectmode=tk.BROWSE, show='headings') 
		...
		tablaGastos.heading("ORDEN", text="ORDEN", command=lambda: treeview_sort_column(tablaGastos, "ORDEN", False))
		tablaGastos.heading("NUMERO", text="NUMERO", command=lambda: treeview_sort_column(tablaGastos, "NUMERO", False))
		tablaGastos.heading("DESCRIPCION", text="DESCRIPCION", command=lambda: treeview_sort_column(tablaGastos, "DESCRIPCION", False))

		tablaGastos.column("ORDEN", width=30)
		tablaGastos.column("NUMERO", width=30)
		tablaGastos.column("DESCRIPCION", width=250)



		tablaGastos.bind("<Double-1>", (lambda event: elegirOrden(tablaGastos.item(tablaGastos.selection()))))
		tablaGastos.bind("<Return>", (lambda event: elegirOrden(tablaGastos.item(tablaGastos.selection()))))
